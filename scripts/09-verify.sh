...
if [[ -n "${VGW_INGRESS_RTB_ID:-}" ]]; then
    ...
    # Check routes to business subnets
...
log_info "Appliance ENI count: $ENI_COUNT"

# Count mirror sessions
SESSION_COUNT=0
if [[ -n "${MIRROR_SESSION_IDS:-}" ]]; then
    IFS=',' read -ra SESSIONS <<< "$MIRROR_SESSION_IDS"
    SESSION_COUNT=${#SESSIONS[@]}
fi
log_info "Mirror session count: $SESSION_COUNT"

if [[ "$ENI_COUNT" -gt 0 && "$SESSION_COUNT" -gt 0 ]]; then
    if [[ "$ENI_COUNT" == "$SESSION_COUNT" ]]; then
        check_pass "ENI count ($ENI_COUNT) matches mirror session count ($SESSION_COUNT)"
    else
        check_fail "ENI count ($ENI_COUNT) != mirror session count ($SESSION_COUNT)"
    fi
else
    check_fail "ENI count ($ENI_COUNT) or session count ($SESSION_COUNT) is zero"
fi
